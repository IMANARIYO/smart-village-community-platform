...
import { VolunteeringCategory, VolunteeringStatus } from "../types";
...
interface VolunteeringEventFiltersProps {
  onFiltersChange: (filters: FilterState) => void;
  filters: FilterState;
}

const statusOptions = Object.values(VolunteeringStatus).map((status) => ({
  value: status,
  label: status.charAt(0).toUpperCase() + status.slice(1).toLowerCase(), // optional formatting
}));

const categoryOptions = Object.values(VolunteeringCategory).map((category) => ({
  value: category,
  label: category, // you could format if needed
}));

export function VolunteeringEventFilters({ onFiltersChange, filters }: VolunteeringEventFiltersProps) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-80 p-x-4 p-y-8 h-[50vh] overflow-y-auto]" align="end">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Input
                ...
                value={filters.organizer_phone || ""}
                onChange={(e) => handleFilterChange("organizer_phone", e.target.value)}
              />
            </div>
